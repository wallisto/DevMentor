...
namespace Dynamic
{
    ...
    /// <summary>
    /// Based on the code found at http://www.codeproject.com/Articles/15845/A-class-to-dynamically-create-delegates-of-functio
    /// </summary>
    public class FastInvoke
    {
        private readonly FastInvokeHandler myDelegate;
    

        public FastInvoke(Type targetType, String methodName)
        {
            MethodInfo methodInfo = targetType.GetMethod(methodName);
            myDelegate = GetMethodInvoker(methodInfo);
        }

        public object ExecuteDelegate(object target , params object[] parameters)
        {
            try
            {
                return (myDelegate(target, parameters));
            }
            catch (Exception e)
            {
                var o = new Object();
