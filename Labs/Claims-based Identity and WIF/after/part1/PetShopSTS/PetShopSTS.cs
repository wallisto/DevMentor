...
namespace DM
{
    public class PetShopSTS : SecurityTokenService
    {
        ...
        protected override Scope GetScope(IClaimsPrincipal principal, RequestSecurityToken request)
        {
            ...
            return new Scope(request, base.SecurityTokenServiceConfiguration.SigningCredentials)
                        {
        ...
        static Dictionary<string, int> experiencePointsMap = new Dictionary<string, int>()
        {
            { "Alice", 50 },
            { "Bob", 100 },
        };
        ...
        protected override IClaimsIdentity GetOutputClaimsIdentity(IClaimsPrincipal principal, RequestSecurityToken request, Scope scope)
        {
            ...
            experiencePointsMap.TryGetValue(identity.Name, out experiencePoints);

            return new ClaimsIdentity(
                new Claim[] 
                {
                    new Claim(WSIdentityConstants.ClaimTypes.Name, identity.Name),
                    new Claim(PetShopClaimTypes.ExperiencePoints, experiencePoints.ToString())
                });
        }
